...
for sysname in sysnames:
    ...
    dfs_c[sysname]	= pd.read_csv(fname_c ,skiprows=29, header=None, delim_whitespace=True)
...
for ii in range(3):
	...
	axs[ii].plot(z_aus[sysname], (df[4]*0.1).tolist(),   color='cyan' , label='OW/10',   lw=2, alpha=0.5)
	...
	axs[ii].plot(z_aus[sysname], df[5].tolist(),         color='lightpink', label='K',     lw=2)
	axs[ii].plot(z_aus[sysname], df[6].tolist(),         color='lime',   	label='Cl',    lw=2)
	axs[ii].set_title(f"{plotnames[sysname]}", fontsize=16 )
	axs[ii].legend(fontsize=12, loc='upper right')
...
for ii in range(3):
	...
	axs[ii].plot(z_aus[sysname], df[4].tolist(),   color='cyan',    label='Water',   lw=2)
	#axs[ii].plot(z_aus[sysname], df[5].tolist(),   color='lightpink',label='K',     lw=2)
	#axs[ii].plot(z_aus[sysname], df[6].tolist(),   color='lime',   	label='Cl',    lw=2)
	axs[ii].set_title(f"{plotnames[sysname]}", fontsize=16 )
	axs[ii].legend(fontsize=12, loc='lower right')
...
axs.plot(z_aus[sysname], (0.1*df[3]).tolist(),         color='cyan',   label='OW/10',     lw=2)
axs.plot(z_aus[sysname], df[4].tolist(),         color='lightpink',   label='K',     lw=2)
axs.plot(z_aus[sysname], df[5].tolist(),         color='lime',   label='Cl',     lw=2)
